...
{
	public class DiscoverMulticastServer : SimpleChannelInboundHandler<DatagramPacket>, IDiscoverServer
	{
		public const int DefaultServerPort = 8999;
		...
		public DiscoverMulticastServer(
			...
			AddressFamily addressFamily = AddressFamily.InterNetwork) : base(true)
		{
			...
			_multicastAddress = multicastAddress ?? DefaultMulticastAddress(addressFamily);
			_group = group;
				...
				.Create<object>(async (observer, cancellationToken) =>
				{
					try
					{
						...
						try
						{
							...
							try
							{
								IPEndPoint multicastEndPoint = new IPEndPoint(_multicastAddress, _discoveryServerPort);
								await serverChannel
									.JoinGroup(multicastEndPoint, NetUtil.LoopbackInterface(_addressFamily))
									.ConfigureAwait(false);
								...
								{
									await serverChannel
										.LeaveGroup(multicastEndPoint, NetUtil.LoopbackInterface(_addressFamily))
										.ConfigureAwait(false);
								}
							}
							finally
